  ...
  module LessConversion
    ...
    def process_stylesheet_assets
      ...
      files.each do |name, file|
        ...
        case name
          when 'base.less'
            file = replace_all file, /(img)&/, '&\1'
        when 'mixins/layout.less'
      ...
      main_from = "#{save_to}/_cardinal.scss"
      main_to   = File.expand_path("#{save_to}/../_cardinal.scss")
      save_file main_to, File.read(main_from).gsub(/ "/, ' "cardinal/')
...
SASS
    ...
    def replace_vars(less)
      ...
      less[from..-1] = less[from..-1].
          gsub(/(?!@mixin|@media|@page|@keyframes|@font-face|@viewport|@-\w)@/, '$').
